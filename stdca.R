stdca <- function(data, outcome, ttoutcome, timepoint, predictors, xstart=0.01, xstop=0.8, xby=0.01, 
  ...
  if (graph==TRUE) {
    ...
    # PLOTTING NET BENEFIT IF REQUESTED
      ...
      plot(x=nb$threshold, y=nb$all, type="l", col=10, lwd=3 ,xlim=c(xstart, xstop), ylim=c(ymin, ymax), xlab="Threshold probability", ylab="Net benefit")
      # adding treat none option
      lines(x=nb$threshold, y=nb$none,lwd=3, col=8)
      ...
      for(m in 1:pred.n) {
        if (smooth==TRUE){
		  ...
		  lines(nb$threshold,data.matrix(nb[paste(predictors[m],"_sm",sep="")]),col=m,lty=1,lwd=3) 
        } else {
        ...
        legendlabel <- c(legendlabel, predictors[m])
        legendcolor <- c(legendcolor, m)
        legendwidth <- c(legendwidth, 3)
        legendpattern <- c(legendpattern, 1)
      }
    }
    # then add the legend
    legend("topright", legendlabel, cex=0.8,  bty = "n",col=legendcolor, lwd=legendwidth, lty=legendpattern)
